...
public class EnumerableSort extends SingleRel implements AccumuloRel, EnumerableRel {
	...
	public EnumerableSort(RelOptCluster cluster, RelTraitSet traits,
			RelNode input,RexNode offset,
			RexNode fetch, AccumuloTable<?> accumuloAccessor) {
...
//		assert getConvention() == CONVENTION;
	...
	public SingleRel copy(RelTraitSet traitSet, List<RelNode> inputs) {
		return new EnumerableSort(getCluster(), traitSet, sole(inputs),
				 fetch, offset, accumuloAccessor);
	}
