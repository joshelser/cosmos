...
public class SortableResult  {
	private static final Logger log = LoggerFactory
			.getLogger(SortableResult.class);

	private static final SortedSet<Text> SPLITS = ImmutableSortedSet.of(
			new Text("0"), new Text("1"), new Text("2"), new Text("3"),
			new Text("4"), new Text("5"), new Text("6"), new Text("7"),
			new Text("8"), new Text("9"));
	...
	public SortableResult(Connector connector, Authorizations auths,
		...
		this(connector, auths, columnsToIndex, Defaults.LOCK_ON_UPDATES,
				Defaults.DATA_TABLE, Defaults.METADATA_TABLE);
	}

	public SortableResult(Connector connector, Authorizations auths,
			Set<Index> columnsToIndex, boolean lockOnUpdates) {
		this(connector, auths, columnsToIndex, lockOnUpdates,
				Defaults.DATA_TABLE, Defaults.METADATA_TABLE);
	}

	public SortableResult(Connector connector, Authorizations auths,
			Set<Index> columnsToIndex, boolean lockOnUpdates, String dataTable,
	...
	protected void createIfNotExists(TableOperations tops, String tableName) {
		if (!tops.exists(tableName)) {
			try {
				...
				// TODO Make a better API than runtimeexception? Do (should) I
				// care?
	...
	/**
	 * Make sure we have a reasonable number of splits for the data table or
	 * else concurrency will just grind to a halt.
	 * 
	 * @param tops
	 * @param tableName
	 */
	protected void splitTable(TableOperations tops, String tableName) {
		try {
			...
			Collection<Text> splits = tops.listSplits(tableName,
					EXPECTED_SPLITS);
	...
	/**
	 * Ensure that the {@link Defaults.CONTENT_LG_NAME} locality group is
	 * configured
	 * 
	 * @param tops
	 * @param tableName
	...
	protected void addLocalityGroups(TableOperations tops, String tableName) {
		try {
			Map<String, Set<Text>> localityGroups = tops
					.getLocalityGroups(tableName);

			// If we don't have a locality group specified with the expected
			// name
			// create one automatically
			if (!localityGroups.containsKey(Defaults.CONTENTS_LG_NAME)) {
				localityGroups.put(Defaults.CONTENTS_LG_NAME,
						Collections.singleton(Defaults.CONTENTS_COLFAM_TEXT));

				tops.setLocalityGroups(tableName, localityGroups);
			} else {
				Set<Text> colfams = localityGroups
						.get(Defaults.CONTENTS_LG_NAME);
				if (!colfams.contains(Defaults.CONTENTS_COLFAM_TEXT)) {
					log.warn(
							"The {} locality group does not contain the expected column family {}",
							Defaults.CONTENTS_LG_NAME,
							Defaults.CONTENTS_COLFAM_TEXT);
				}
			}
		} catch (AccumuloException e) {
			log.error("Could not add locality groups to table '{}'", tableName,
					e);
			throw new RuntimeException(e);
		} catch (AccumuloSecurityException e) {
			log.error("Could not add locality groups to table '{}'", tableName,
	...
	protected void addColumnsToIndex(Collection<Index> columns) {
		...
		} else if (!(IdentitySet.class.isAssignableFrom(this.columnsToIndex
				.getClass()))) {
	...
	public static SortableResult create(Connector connector,
			Authorizations auths, Set<Index> columnsToIndex) {
		return new SortableResult(connector, auths, columnsToIndex);
	}

	public static SortableResult create(Connector connector,
			Authorizations auths, Set<Index> columnsToIndex,
			boolean lockOnUpdates) {
		return new SortableResult(connector, auths, columnsToIndex,
				lockOnUpdates);
	}
	...
	public static SortableResult create(Connector connector,
			Authorizations auths, Set<Index> columnsToIndex,
			boolean lockOnUpdates, String dataTable, String metadataTable) {
		return new SortableResult(connector, auths, columnsToIndex,
				lockOnUpdates, dataTable, metadataTable);
	}
