...
public class LimitRule extends ConverterRule {
	...
	public void onMatch(RelOptRuleCall call) {
		...
		if (!sort.getCollation().getFieldCollations().isEmpty()) {
			input = sort.copy(sort.getTraitSet()
					.replace(RelCollationImpl.EMPTY), input,
					RelCollationImpl.EMPTY, null, null);
		}
		
		final RelNode convertedInput =  convert(
		          input,
		          input.getTraitSet().plus(
		        		  AccumuloRel.CONVENTION));
		
		
		call.transformTo(new EnumerableSort(sort.getCluster(), traits,
				convertedInput,sort.fetch, sort.offset,
				accumuloAccessor));
	...
	public RelNode convert(RelNode rel) {
		...
		return new EnumerableSort(sort.getCluster(), traits,
				convertedInput,sort.fetch, sort.offset,
				accumuloAccessor);
	}
